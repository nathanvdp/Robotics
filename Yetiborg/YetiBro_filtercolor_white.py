...
try:
    while(True):
        ...
        #result_white = cv2.bitwise_and(crop_img, crop_img, mask=mask_white)
        #ret_white, thresh_white = cv2.threshold(result_white,1,255,cv2.THRESH_BINARY)

        # Find the contours of the frame
        contours,hierarchy = cv2.findContours(mask_white.copy(), 1, cv2.CHAIN_APPROX_NONE)
